...
class SuperAdminController extends Controller
{
    ...
    /**
     * Get system-wide purchase orders via API (superadmin only)
     */
    public function getSystemPurchaseOrdersApi(Request $request)
    {
        try {
        ...
        $statusId = (string) $request->query('status', '');
        $supplierId = (string) $request->query('supplier_id', '');

        $dateFrom = (string) $request->query('date_from', '');
        $dateTo = (string) $request->query('date_to', '');

        $minTotal = $request->query('min_total', null);
        $maxTotal = $request->query('max_total', null);
        ...
        $cacheKey = 'superadmin_pos_v1_'.md5(json_encode([
            'page' => $page,
            'per_page' => $perPage,
            'search' => $search,
            'status' => $statusId,
        ...
        $load = function () use ($perPage, $page, $search, $statusId, $supplierId, $dateFrom, $dateTo, $minTotal, $maxTotal, $sortBy, $sortDir) {
            ...
            if ($statusId !== '') {
                $query->where('st.status_id', $statusId);
            }
            ...
            if ($dateFrom !== '') {
                $query->whereDate('po.date_requested', '>=', $dateFrom);
            }
            if ($dateTo !== '') {
                $query->whereDate('po.date_requested', '<=', $dateTo);
            }

            if ($minTotal !== null && $minTotal !== '') {
                $query->where('po.total', '>=', (float) $minTotal);
            }
            if ($maxTotal !== null && $maxTotal !== '') {
                $query->where('po.total', '<=', (float) $maxTotal);
            }

            if ($sortBy === 'supplier_name') {
                $query->orderBy('s.name', $sortDir);
            } elseif ($sortBy === 'status_name') {
                $query->orderBy('st.status_name', $sortDir);
            } else {
                $query->orderBy('po.'.$sortBy, $sortDir);
            }
        ...
        try {
            $paginator = $noCache ? $load() : Cache::remember($cacheKey, 15, $load);

            $filters = Cache::remember('superadmin_pos_filters_v1', 300, function () {
                $statuses = Schema::hasTable('statuses')
                    ? DB::table('statuses')->orderBy('status_name')->get(['status_id', 'status_name', 'color'])
                    : collect([]);

                $suppliers = Schema::hasTable('suppliers')
                    ? DB::table('suppliers')->orderBy('name')->get(['supplier_id', 'name'])
                    : collect([]);
            ...
            Log::error('System purchase orders API failed', [
                'error' => $e->getMessage(),
    ...
    public function updateBranding(Request $request)
    {
        ...
        if ($request->hasFile('logo')) {
            try {
                ...
                if ($oldLogo) {
                    if (str_starts_with($oldLogo, '/storage/')) {
                        $oldPath = str_replace('/storage/', 'public/', $oldLogo);
                        if (Storage::exists($oldPath)) {
                            Storage::delete($oldPath);
                        }
                    } elseif (str_starts_with($oldLogo, '/branding/')) {
                        $candidate = public_path(ltrim($oldLogo, '/'));
                ...
                $name = 'logo_'.Str::uuid()->toString().'.'.$ext;
                $file->move($dir, $name);
                $public = '/branding/'.$name;
                ...
                Log::warning('Branding logo upload failed', ['error' => $e->getMessage()]);
        ...
        switch ($action) {
            ...
            case 'restore_backup':
                try {
                    ...
                    if (! $file) {
                        return back()->withErrors(['backup_file' => 'No file uploaded.']);
                    }
                    $json = null;
                    $mime = $file->getMimeType();
                    if (str_contains((string) $mime, 'zip')) {
                        if (! class_exists(\ZipArchive::class)) {
                            return back()->withErrors(['backup_file' => 'Zip support is not available on the server.']);
                        }
                        $zip = new \ZipArchive;
                        ...
                        if ($idx !== false) {
                            ...
                            for ($i = 0; $i < $zip->numFiles; $i++) {
                                ...
                                if (is_string($name) && str_ends_with(strtolower($name), '.json')) {
                        ...
                        $zip->close();
                        if (! $jsonContent) {
                            return back()->withErrors(['backup_file' => 'No JSON data found inside ZIP.']);
                        }
                        $json = $jsonContent;
                    } else {
                    ...
                    if (! is_array($data)) {
                        return back()->withErrors(['backup_file' => 'Invalid JSON structure.']);
                    }
                    ...
                    DB::transaction(function () use ($tablesData, $wipe) {
                        if ($wipe) {
                            ...
                            foreach (['items', 'approvals', 'roles', 'login', 'purchase_orders', 'users', 'suppliers'] as $t) {
                                if (array_key_exists($t, $tablesData)) {
                                    DB::table($t)->delete();
                                }
                            }
                            // Independent tables
    ...
    public function createUser(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'username' => ['required', 'string', 'min:3', 'max:50', 'regex:/^[a-zA-Z0-9_]+$/', 'unique:login,username'],
        ...
        try {
            ...
            if (Schema::hasColumn('users', 'position')) {
                $userRow['position'] = $request->position ?? '';
            }
            if (Schema::hasColumn('users', 'department')) {
                $userRow['department'] = $request->department ?? '';
            }
            ...
            if (Schema::hasTable('login')) {
                ...
                if (Schema::hasColumn('login', 'created_at')) {
                    $loginRow['created_at'] = now();
                }
                if (Schema::hasColumn('login', 'updated_at')) {
                    $loginRow['updated_at'] = now();
                }
                DB::table('login')->insert($loginRow);
            }
            ...
            if (Schema::hasTable('role_types') && Schema::hasTable('roles')) {
                ...
                if ($roleTypeId) {
                    ...
                    $roleTypeId = (string) \Illuminate\Support\Str::uuid();
            ...
            Log::error('User create failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
    ...
    public function createUserApi(Request $request)
    {
        ...
        try {
            ...
            Log::error('User create API failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
    ...
    /**
     * Get security statistics via API
     */
    public function getSecurityStatsApi(Request $request)
    {
        try {
            $this->requireSuperadmin($request);
        } catch (\Throwable $e) {
        ...
        try {
            $activeSessionsCount = $this->getActiveSessionsCountSafely();

            $activitiesLast24h = 0;
            if (Schema::hasTable('system_activity_logs')) {
                $activitiesLast24h = (int) DB::table('system_activity_logs')
                    ->where('created_at', '>=', now()->subHours(24))
                    ->count();
            } elseif (Schema::hasTable('login') && Schema::hasColumn('login', 'created_at')) {
                $activitiesLast24h = (int) DB::table('login')
                    ->where('created_at', '>=', now()->subHours(24))
                    ->count();
            }
            ...
            if (Schema::hasTable('security_alerts')) {
                $unresolvedAlerts = (int) DB::table('security_alerts')->where('is_resolved', false)->count();
                $criticalAlerts = (int) DB::table('security_alerts')->where('is_resolved', false)->where('severity', 'critical')->count();
            }

            // Note: project does not currently persist failed login attempts; keep this best-effort.
            $loginSuccessRate = 100.0;
            ...
            Log::error('Security stats API failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'admin' => session('auth_user.username', 'system'),
            ]);
    ...
    /**
     * Get security alerts via API
     */
    public function getSecurityAlertsApi(Request $request)
    {
        try {
            $this->requireSuperadmin($request);
        } catch (\Throwable $e) {
        ...
        try {
            ...
            if (Schema::hasTable('security_alerts')) {
                ...
                $alerts = $query->get([
                    'id', 'alert_type', 'severity', 'title', 'description', 'user_id', 'ip_address', 'created_at',
                ])->toArray();
            } else {
                // Fallback: use computed alerts (legacy helper)
            ...
            Log::error('Security alerts API failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'admin' => session('auth_user.username', 'system'),
            ]);
